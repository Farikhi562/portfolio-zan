...
export default function Navbar() {
  ...
  return (
    <>
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? 'glass shadow-sm shadow-black/5 py-2.5' : 'bg-transparent py-4'}`}>
        <nav className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-3">
          ...
          <div className="hidden xl:flex"><LiveClock /></div>
          ...
          <ul className="hidden md:flex items-center gap-0.5">
            {NAV_LINKS.map(link => {
              ...
              return (
                <li key={link.href}>
                  <Link href={link.href}
                    className={`relative px-3 py-2 text-sm font-semibold rounded-xl transition-all ${active ? 'text-blue-600 bg-blue-50' : 'hover:bg-[var(--surface-raised)]'}`}
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={toggle} aria-label="Toggle theme"
              className="w-9 h-9 rounded-xl flex items-center justify-center border transition-all hover:scale-105 hover:border-blue-300"
            ...
            <Link href="/contact"
              className="hidden md:inline-flex items-center gap-1.5 bg-blue-600 text-white text-xs sm:text-sm font-bold px-4 py-2.5 rounded-xl hover:bg-blue-700 transition-all shadow-md shadow-blue-200">
              Hire Me
            </Link>
      ...
      >
        <div className="flex flex-col h-full p-5 overflow-y-auto">
          <div className="flex justify-between items-center mb-5">
            <span className="font-(family-name:--font-syne) font-black text-sm" style={{ color: 'var(--text)' }}>Menu</span>
            <button onClick={() => setMenuOpen(false)} className="w-8 h-8 rounded-xl flex items-center justify-center border" style={{ borderColor: 'var(--border)', color: 'var(--text-3)' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>
            </button>
          </div>
          ...
          <div className="my-4"><GitHubWidget /></div>
